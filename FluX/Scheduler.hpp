...
namespace flux {
    ...
    public:
        ...
        /// Number of worker threads. 0 if Run() hasn't been called yet.
        size_t GetWorkerCount() const;

        /// Cumulative busy time (ns) for one worker since it started -- see
        /// Worker::busy_ns / Workers::GetBusyNs. Callers diff two samples
        /// against elapsed wall time to get instantaneous utilization (this is
        /// exactly what FTXUIDashboard does on every render tick). Returns 0 if
        /// Run() hasn't been called or worker_index is out of range.
        uint64_t GetWorkerBusyNs(size_t worker_index) const;
